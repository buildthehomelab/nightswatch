  ...
  if (load1 != null && load1 >= LOAD_WARN) {
    ...
    issues.push({
      ...
      detail: {
        type: 'load',
        load1,
        load5: data.info.loadavg[1] ?? null,
        load15: data.info.loadavg[2] ?? null,
        cores,
        pct,
        trend: trend.trim(),
        runningApps,
        jobs: jobs.map(j => ({
          method: j.method,
          pct: j.progress?.percent != null ? Math.round(j.progress.percent) : null,
          desc: j.progress?.description ?? null,
        })),
        warnAt: LOAD_WARN,
        critAt: LOAD_CRIT,
      },
