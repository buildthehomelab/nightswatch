// dozzle.jsx — embedded Dozzle-style log viewer overlay
const { useState: useDState, useEffect: useDEffect, useRef: useDRef, useMemo: useDMemo } = React;
...
// ── Mock log streams (per container) ───────────────────
const LOG_TEMPLATES = {
  sonarr: [
...
function Dozzle({ open, onClose, initialContainer }) {
  const [active, setActive] = useDState(initialContainer || "sonarr");
  const [filter, setFilter] = useDState("");
  const [showInfo, setShowInfo] = useDState(true);
  const [showWarn, setShowWarn] = useDState(true);
  const [showErr,  setShowErr]  = useDState(true);
  const [lines, setLines] = useDState(() => buildInitialLines(active));
  const streamRef = useDRef(null);

  // when switching container, rebuild
  useDEffect(() => {
  ...
  // simulate live tail when overlay is open
  useDEffect(() => {
    if (!open) return;
...
window.Dozzle = Dozzle;
window.DOZZLE_CONTAINERS = DOZZLE_CONTAINERS;
